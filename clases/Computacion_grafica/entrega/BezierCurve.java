...
private void drawPoints(Graphics g) {
    for (int i = 0; i < controlPoints.length; i++) {
        ...
        int z = (int) controlPoints[i].z;
        drawFatPoint(x, y, z, g);

        }
    }
    
private void drawFatPoint(int x, int y, int z, Graphics g) {
    x = x + FRAME_WIDTH / 2;
    y = FRAME_HEIGHT / 2 - y;
    g.setColor(Color.black);
    g.drawOval(x - 1, y - 1, 3, 3);
    // Consider z-coordinate while drawing (for a basic representation)
    g.drawLine(x, y, x + z, y);
    }
    ...
    private void drawAxis(Graphics g) {
        ...
        drawEdge(g,new Point(0.0, -100.0,0.0), new Point(0.0, 100.0,0.0));
        g.setColor(Color.green);
        drawEdge(g,new Point(-100.0, 0.0,0.0), new Point(100.0, 0.0,0.0));
    }
...
private void drawEdge(Graphics g, Point p0, Point p1) {
    ...
    // Draw line with the z-coordinate
    g.drawLine(x0, y0, x1 , y1);
//    g.drawLine(x0, y0, x1 + (int) p1.z, y1);
